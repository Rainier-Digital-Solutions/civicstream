...
import { reviewPlanWithResponsesAPI } from '@/lib/openai';
...
export async function POST(req: NextRequest) {
    console.log('[ResponsesAPI] Received plan review request');
    ...
    try {
        ...
        console.error('[ResponsesAPI] Error handling request:', error);
...
async function handleDirectUpload(req: NextRequest) {
    try {
        ...
        if (!submitterEmail || !cityPlannerEmail || !address || !parcelNumber || !city || !county) {
            console.error('[ResponsesAPI] Missing required fields');
            return NextResponse.json(
        ...
        console.log('[ResponsesAPI] Writing file to temp location:', tempFilePath);
        fs.writeFileSync(tempFilePath, buffer.toString('binary'), 'binary');
        ...
        try {
            ...
            // 3. Now you can use file_search with that vector store ID
            const response = await openai.responses.create({
                ...
                input: "Extract information from my document",
            });

            // Process the file with the Responses API
            ...
            // Call the Responses API function
            console.log('[ResponsesAPI] Processing PDF with Responses API');
            const reviewResult = await reviewPlanWithResponsesAPI(
                buffer,
                file.name,
                projectDetails
            );

            console.log('[ResponsesAPI] Review completed:', {
            ...
            console.log('[ResponsesAPI] Sending email:', {
                baseUrl,
                submitterEmail,
                cityPlannerEmail,
                isCompliant: reviewResult.isCompliant
            ...
            if (!emailResponse.ok) {
                console.error('[ResponsesAPI] Failed to send email:', {
            ...
            try {
                if (fs.existsSync(tempFilePath)) {
                    ...
                    console.log('[ResponsesAPI] Temp file deleted:', tempFilePath);
                }
            } catch (cleanupError) {
                console.error('[ResponsesAPI] Error cleaning up temp file:', cleanupError);
            }
        }
    } catch (error) {
        console.error('[ResponsesAPI] Error in direct upload handling:', error);
        return NextResponse.json(
...
async function handleBlobUrlUpload(req: NextRequest) {
    try {
        ...
        if (!blobUrl) {
            console.error('[ResponsesAPI] Missing blobUrl');
            return NextResponse.json(
        ...
        console.log('[ResponsesAPI] Fetching file from Blob:', blobUrl);
        ...
        try {
            ...
            try {
                await del(blobUrl);
                console.log('[ResponsesAPI] Blob deleted:', blobUrl);
            } catch (error) {
                console.error('[ResponsesAPI] Error deleting blob:', error);
            }
            ...
            console.error('[ResponsesAPI] Error fetching or processing file:', error);
            return NextResponse.json(
        ...
        console.error('[ResponsesAPI] Error in blob URL handling:', error);
        return NextResponse.json(
